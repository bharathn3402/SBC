...
void FS65_ErrorCallback(void) {
    ...
    for (;;)
    {
    	PIT_wait_micsec(1000);  //Wait 1sec
...
uint32_t FS65_Init_FSSM(void)
{
    ...
    PIT_wait_micsec(3);  //Wait 3micsec
    ...
    PIT_wait_micsec(3);                                          //no CAN Tx when FS1b is activated
...
void FS65_Init(void) {
	...
	PIT_wait_micsec(200);  //Wait 200micsec
 ...
 *		operation for the LDT. Resolutions is 488µsec
 *   @par Include
 *		FS65xx.h
 *   @par Description
...
uint32_t FS65_SendCmdR(uint32_t cmd){
    ...
    INTC_0.CPR0.B.PRI = INT_CEIL_PRIORITY;		//block DSPI resource
    ...
    //DSPI_Send(DSPI_NB, DSPI_CS, 0xABBA);
	...
	if(SPIstruct.response == 0xFFFF){
		...
		INTC_0.CPR0.B.PRI = stockPriority;					//release DSPI resource
